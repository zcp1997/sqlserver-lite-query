...
export default function WorkspaceSelector({
  ...
  const [workspaceComboOpen, setWorkspaceComboOpen] = useState(false)
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <Popover open={workspaceComboOpen} onOpenChange={setWorkspaceComboOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            role="combobox"
            aria-expanded={workspaceComboOpen}
            className="w-[280px] justify-between"
            disabled={!activeSession || isInitializing}
          >
            {selectedWorkspace
              ? formatWorkspaceName(selectedWorkspace)
              : (isInitializing ? "加载中..." : "选择工作区")
            }
            ...
            <CommandList>
              ...
              <CommandGroup>
                {workspaceManager.workspaces.map((workspace) => (
                  <CommandItem
                    key={workspace.id}
                    value={formatWorkspaceName(workspace)}
                    onSelect={() => {
                      handleWorkspaceChange(workspace.id)
                      setWorkspaceComboOpen(false)
                    }}
                  >
                    <Check
                      className={`mr-2 h-4 w-4 ${selectedWorkspace?.id === workspace.id ? "opacity-100 text-green-500" : "opacity-0"
                        }`}
                    />
                    <div className="flex items-center">
                      <FolderOpen className={`h-4 w-4 mr-2 ${selectedWorkspace?.id === workspace.id ? "text-green-500" : ""}`} />
                      <span className={selectedWorkspace?.id === workspace.id ? "text-green-500 font-medium" : ""}>
